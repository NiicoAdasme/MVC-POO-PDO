<header class="header">
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        <div id="mySidenav" class="sidenav">
        <a class="text-white" href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
        <a class="display-3 text-white" href="index.php">&#127969;Inicio</a>
        <a class="display-3 text-white" href="Vista/usuarios.php">&#128218;Usuarios</a>
        <a class="display-3 text-white" href="">&#127760;Libros</a>
        <!--<a class="display-3 text-white" href="#">&#128113;Alumnos</a>
        <a class="display-3 text-white" href="#">&#127936;Actividades Extra Programaticas</a>
        <a class="display-3 text-white" href="#">&#127969;Ciudad</a>
        <a class="display-3 text-white" href="#">&#128236;Región</a>
        <a class="display-3 text-white" href="#">&#128208;Curso</a>
        <a class="display-3 text-white" href="#">&#128197;Bloque Horario</a>
        <a class="display-3 text-white" href="#">&#128203;Nivel Curso</a>-->
    </div>
    <span class="text-white pr-5" style="font-size:30px;cursor:pointer" onclick="openNav()">&#9776; Menú</span>
    <a class="navbar-brand" href="index.php"><strong>Pruebas Con POO - MVC - PDO</strong></a>
    </nav>
  </header>

  